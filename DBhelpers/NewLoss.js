...
exports.NewLoss = async (data) => {
    ...
    if(data.returnValues.matrix === '1'){
        ...
        foundX3.loss += price[Number(data.returnValues.level-1)]

        await user.findOneAndUpdate({ address: data.returnValues.receiver }, { $inc: { 'totalloss': price[Number(data.returnValues.level - 1)] } }).exec((e, r) => { if (e) { console.log(e) } })
    ...
    if (data.returnValues.matrix === '2') {
        ...
        foundX6.loss += price[Number(data.returnValues.level - 1)]
        await user.findOneAndUpdate({ address: data.returnValues.receiver }, { $inc: { 'totalloss': price[Number(data.returnValues.level - 1)] } }).exec((e, r) => { if (e) { console.log(e) } })
